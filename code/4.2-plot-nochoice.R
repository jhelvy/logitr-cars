...
load(here("models", "mnl_nochoice.RData"))

# -----------------------------------------------------------------------------
# Plot results

# Get the estimated coefficients
coefs <- coef(mnl_nochoice)
...
df_nochoice = data.frame(level = c("No", "Yes")) %>%
...
utility <- c(
    ...
    df_accelTime$utility, df_powertrain$utility, df_nochoice$utility)
...
plots_nochoice <- plot_grid(
  plot_price, plot_fuelEconomy, plot_accelTime, plot_powertrain, plot_nochoice,
  nrow = 1
)

# Save plots 
ggsave(
  filename = here('figs', 'mnl_nochoice.png'),
  plot = plots_nochoice,
  width = 12, height = 2.3
)
